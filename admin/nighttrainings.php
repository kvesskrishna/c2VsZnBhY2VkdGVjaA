...
if (isset($_POST['deletetraining'])) {
	# code...
	$training_id=$_POST['training_id'];
	$sql_deletedata="DELETE FROM trainings WHERE training_id=$training_id";
	$res_deletedata=$mysqli->query($sql_deletedata);
	if (!$res_deletedata) {
		$_SESSION['message']=$alert="Training Data Deleted Successfully";
	header('Location:trainings.php');
	}
}
?>
				...
				<div class="main-content-inner">
					<div class="breadcrumbs ace-save-state" id="breadcrumbs">
						<ul class="breadcrumb">
							...
							<li class="active">Night Trainings</li>
						</ul><!-- /.breadcrumb -->
					</div>
...
?>
                    ...
                    <?php } ?>
						            ...
						            <div class="modal-footer">
						                ...
						                <button type="submit" id="addbtn" name="addtraining" class="btn btn-primary">Add Entry</button>
						            </div>
            				</form>
				        </div>
				    </div>
	...
	<table id="ordersa" width="100%" cellspacing="0" class="display compact" width="70%">
        ...
        <tbody>
        <?php
        $sql_getdata="SELECT * FROM trainings WHERE training_time='Night'";
        ...
        while ($row_getdata=$res_getdata->fetch_assoc()) {
        	...
        	<tr>
               ...
               <td><?php if($untraining_update==1) {?><a href="#edit<?php echo $row_getdata['training_id']?>" data-toggle="modal"><i class="fa fa-pencil-square-o"></i></a><?php } if($untraining_delete==1) {?>
                ...
                <div id="delete<?php echo $row_getdata['training_id']?>" class="modal fade">
							...
							<form class="form-horizontal" method="post" action="#">            				
								Delete Request for <b><?php echo $row_getdata['technology']?></b>
						            ...
						            <div class="modal-footer">
						                <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
						                <button type="submit" id="deletebtn" name="deletetraining" class="btn btn-primary">Delete Entry</button>
						            </div>
            				</form>
				        </div>
				    </div>
...
?>
                    ...
                    <?php } ?>
                                    ...
                                    <div class="modal-footer">
                                        <button type="button" class="btn btn-danger" data-dismiss="modal">Close</button>
                                        <button type="submit" id="editbtn" name="edittraining" class="btn btn-primary">Edit Entry</button>
                                    </div>
            				</form>
				        </div>
				    </div>
    ...
    <p>
      <?php if ($untraining_delete==1){?>
        <form class="form-horizontal" method="post" action="exportnighttrainings.php">
